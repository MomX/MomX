#' Conflicts between MomX and other packages
#'
#' This function lists all the conflicts between packages in MomX
...
MomX_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", MomX_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "MomX_conflicts")
}

MomX_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "MomX_conflicts()"
  )
...
#' @export
print.MomX_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(MomX_conflict_message(x))
}
