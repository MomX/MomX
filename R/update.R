...
#'
#' This will check to see if all MomX packages (and optionally, their
...
#' MomX_update()
#' }
MomX_update <- function(recursive = FALSE) {

  deps <- MomX_deps(recursive)
...
#' List all MomX dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'   MomX packages.
#' @export
MomX_deps <- function(recursive = FALSE) {
  pkgs <- utils::available.packages()
  deps <- tools::package_dependencies("MomX", pkgs, recursive = recursive)

  pkg_deps <- unique(sort(unlist(deps)))
  # case when not installed yer
  if (is.null(pkg_deps))
    return(NULL)
  base_pkgs <- c(
